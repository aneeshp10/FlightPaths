...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			this.dataSequence = new LinkedList<>();
			for (T addData : source.dataSequence) {
				this.dataSequence.add(addData);
			}
			this.end = source.end;
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		PriorityQueue<Path> queueNode = new PriorityQueue<>(); // create a priority queue
		ArrayList<Vertex> verticesCollection = new ArrayList<Vertex>(); // create an arraylist to keep track of nodes
																		// that
																		// have been taken into account
		// check if start data is valid, throw exception if not
		if (!containsVertex(start)) {
			throw new NoSuchElementException("Invalid Start Data");

		}
		// check if end data is valid, throw exception if not
		if (!containsVertex(end)) {
			throw new NoSuchElementException("Invalid End Data");

		}

		Vertex pollingNode = vertices.get(start);
		Vertex lastEl = vertices.get(end); // store the Vertex (value) in variable
		Path output = new Path(pollingNode); // initialize the variable to be returned
		verticesCollection.add(pollingNode); // list comprising of visited nodes, should not exceed total vertices count
												// at
												// any point

		// perform repeating steps for all nodes until all nodes are relaxed
		while ((verticesCollection.size() <= vertices.size()) && pollingNode != lastEl) {
			Path endRoute = null;
			boolean checkNull;
			for (Edge vertDistWeight : pollingNode.edgesLeaving) {
				boolean hasTarget = verticesCollection.contains(vertDistWeight.target);
				int dist = output.distance; // store distance of the path object
				int edgeSize = vertDistWeight.weight; // store edge weight

				// check if list contains the current target, enter if block if true and
				// traverse through queue
				if (hasTarget) {
					for (Path tempPath : queueNode) {
						Vertex lastVertex = vertDistWeight.target; // store iterating path object's end vertex
						int tempDist = tempPath.distance; // store current distance of the vertex from the source node
						if ((dist + edgeSize) < tempDist) {
							Vertex tempVert = tempPath.end;
							if (tempVert.equals(lastVertex)) {
								endRoute = tempPath;
							}
						}
					}

					// check if endRoute is null, set boolean flag to false is true
					if (endRoute == null) {
						checkNull = false;
					} else {
						checkNull = true; // else set to true
					}

					// only if checkNull is true enter if block, create extension of path
					if (checkNull) {
						Path extendPath;
						if (queueNode.remove(endRoute)) {
							// extend the path
							if (output != null) {
								extendPath = new Path(output); // create an extension of current output path
								extendPath.extend(vertDistWeight); // "extend"
								queueNode.add(extendPath); // queue to priority queue
				...
				// enter else block if start vertex is equal to start in terms of reference (not
				// in seen vertices list)
				else {
					Path extendPath;
					verticesCollection.add(vertDistWeight.target);
					extendPath = new Path(output); // create an extension of current output path
					extendPath.extend(vertDistWeight); // "extend"
					queueNode.add(extendPath); // queue to priority queue
				}
			...
			// check if priority queue is empty or list has exceeded total number of
			// vertices
			if ((verticesCollection.size() > vertices.size()) || queueNode.isEmpty()) {
				throw new NoSuchElementException(); // throws NullPointerException
			}

			output = queueNode.peek(); // store value at top
			queueNode.remove(); // eliminate block from queue -- not needed
			pollingNode = output.end;

			// throw exception if the the node being removed is the same as the start
			if (pollingNode == output.start) {
	...
	public static void main(String[] args) {
		...
		System.out.println(graph.getPathCost(1, 5));
		System.out.println(graph.getPathCost(2, 4));

		// System.out.println(graph.getWeight(4, 6));
